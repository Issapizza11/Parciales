...
// Colisión + propagación (Push scheme)
#pragma omp parallel for collapse(2)
        for (int i = 0; i < Nx; ++i)
        {
            for (int j = 0; j < Ny; ++j)
...
#pragma omp parallel for
    ...
    std::cout << "Se usaron " << omp_get_max_threads() << " hilos.\n";

    if (!report_mode)
    {
        std::ofstream file("lbm2d_output_parallel.csv");
        file << "x,y,Tg,Ts\n";
        file.precision(10);
        ...
        std::cout << "Simulación 2D LBM completada. Resultados en 'lbm2d_output_parallel.csv'.\n";
    }
